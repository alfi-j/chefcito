...
export function WorkstationList({ workstations, loading, error, onAdd, onUpdate, onDelete, onReorder }: WorkstationListProps) {
  ...
  const handleDrop = async (e: React.DragEvent<HTMLTableRowElement>, dropWorkstationId: string) => {
    e.preventDefault()
    ...
    const fromIndex = workstations.findIndex(w => w.id === draggedWorkstationId)
    const toIndex = workstations.findIndex(w => w.id === dropWorkstationId)
